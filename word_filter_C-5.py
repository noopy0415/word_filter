class WordFilter(object):
    ...
    def ng_word_update(self, ng_word, replace_word):
        if ng_word in self.replace_word:
            self.replace_word.update({ng_word: replace_word})
...
if __name__ == "__main__":
    ...
    print(f"現在のNGワードと変換する文字 : {my_filter.replace_word}")
    ng_word = input("変更するNGワードを入力してください > ")  # アツ
    replace_word = input("変換する文字を入力してください > ")  # 寒
    my_filter.ng_word_update(ng_word, replace_word)
    print(my_filter.replace("昨日のアーセナルの試合アツかった！"))  # 昨日の<censored>の試合寒かった！
    print(my_filter.replace("昨日のリバプールの試合アツかった！"))  # 昨日のリバプールの試合寒かった！
